 ...
 */
class AppLinearSnapHelper(private var gravity: Int, enableSnapLastItem: Boolean, snapListener: SnapListener?) : LinearSnapHelper() {

    val FLING_DISTANCE_DISABLE = -1
    val FLING_SIZE_FRACTION_DISABLE = -1f
    private var isRtl = false
    private var snapLastItem = enableSnapLastItem
    ...
    private var listener: SnapListener? = snapListener
    private var recyclerView: RecyclerView? = null
    private val scrollListener: RecyclerView.OnScrollListener = object : RecyclerView.OnScrollListener() {
        override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
            super.onScrollStateChanged(recyclerView, newState)
            this@AppLinearSnapHelper.onScrollStateChanged(newState)
        }
    }
        ...
        for (i in 0 until lm.childCount) {
            ...
            currentViewDistance = if (snapToStart) {
                if (!snapToPadding) {
                    ...
                    Math.abs(helper.startAfterPadding
                            - helper.getDecoratedStart(currentView))
                }
            } else if (snapToEnd) {
                if (!snapToPadding) {
                    Math.abs(helper.getDecoratedEnd(currentView)
                            - helper.end)
                } else {
                    Math.abs(helper.endAfterPadding
