 ...
 * @author panxuqin
 */
class JustifyTextView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : AppCompatTextView(context!!, attrs, defStyleAttr) {
    /**
     * 绘制文字的起始Y坐标
     */
     ...
     * 当前所有行数的集合
     */
    private var mParagraphLineList: ArrayList<List<List<String>>>? = null
     ...
     * 每一段单词的内容集合
     */
    private var mParagraphWordList: ArrayList<List<String>>? = null
     ...
     * 英语单词元音字母集合
     */
    private val vowels = listOf(*vowel)
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        setMeasuredDimension(mViewWidth, (mParagraphLineList!!.size - 1) * paragraphSpacing + mLineCount * lineHeight)
    }
    ...
    private fun getLineList(frontList: List<String>): List<List<String>> {
        ...
        val lineLists: MutableList<List<String>> = ArrayList()
                    ...
                    if (length <= 3) {
                        ...
                        for (j in 0 until length) {
                            ...
                            if (vowels.contains(tempLastWord)) {
                                // 根据元音字母来进行截断
                                if (j + 1 < length) {
                                    val nextTempLastWord = lastWord[j + 1].toString()
                                    sb.append(nextTempLastWord)
                                    width = StaticLayout.getDesiredWidth(sb.toString(), paint)
                                    cutoffIndex = j
                                    if (width > mViewWidth) {
                                        lastTemp = if (j > 2 && j <= length - 2) {
                                            // 单词截断后，前面的字符小于2个时，则不进行截断
                                            val lastFinalWord = lastWord.substring(0, cutoffIndex + 2) + "-"
                            ...
                            if (width > mViewWidth) {
                                lastTemp = if (j > 2 && j <= length - 2) {
                                    // 单词截断后，前面的字符小于2个时，则不进行截断
                                    val lastFinalWord = lastWord.substring(0, j) + "-"
                                    lineList.add(lastFinalWord)
                                    addLines(lineLists, lineList)
        ...
        Log.i(TAG, "getLineList $lineLists")
        return lineLists
    }
     ...
     */
    private fun addLines(lineLists: MutableList<List<String>>?, lineList: MutableList<String>?) {
        if (lineLists == null || lineList == null) {
            return
        }
        val tempLines: List<String> = ArrayList(lineList)
    ...
    private val paragraphList: Unit
        private get() {
            val text = text.toString().replace("  ".toRegex(), "").replace("   ".toRegex(), "").replace("\\r".toRegex(), "").trim { it <= ' ' }
            mLineCount = 0
            val items = text.split("\\n".toRegex()).toTypedArray()
            mParagraphLineList = ArrayList()
    ...
    private fun getWordList(text: String): List<String> {
        ...
        val frontList: MutableList<String> = ArrayList()
        ...
        for (i in 0 until text.length) {
            ...
            if (charAt != BLANK) {
                ...
                if (!TextUtils.isEmpty(str.toString())) {
                    frontList.add(str.toString().replace(BLANK.toRegex(), ""))
                    str.delete(0, str.length)
                }
            }
        }
    ...
    private fun adjust(canvas: Canvas, paint: TextPaint) {
        ...
        for (j in 0 until size) { // 遍历每一段
            val lineList = mParagraphLineList!![j]
     ...
     * @param paint
     */
    private fun drawScaledEndText(canvas: Canvas?, lineWords: List<String>?, paint: TextPaint?) {
        ...
        if (TEXT_ALIGNMENT_TEXT_START == textAlignment || TEXT_ALIGNMENT_VIEW_START == textAlignment || Gravity.LEFT == lastGravity) {
            canvas.drawText(sb.toString(), 0f, mLineY, paint)
        } else if (TEXT_ALIGNMENT_TEXT_END == textAlignment || TEXT_ALIGNMENT_VIEW_END == textAlignment || Gravity.RIGHT == lastGravity) {
     ...
     * @param paint
     */
    private fun drawScaledText(canvas: Canvas?, line: List<String>?, paint: TextPaint?) {
    ...
    companion object {
        private val TAG = JustifyTextView::class.java.simpleName
